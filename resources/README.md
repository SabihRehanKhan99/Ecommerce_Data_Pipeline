...
The databricks CLI bundles top level is defined by file `ecommerce_user_behaviour/databricks.yml`.
...
ML Resource Configurations in this directory:
 - model workflow (`ecommerce_user_behaviour/resources/model-workflow-resource.yml`)
 - batch inference workflow (`ecommerce_user_behaviour/resources/batch-inference-workflow-resource.yml`)
 - monitoring resource and workflow (`ecommerce_user_behaviour/resources/monitoring-resource.yml`)
 - feature engineering workflow (`ecommerce_user_behaviour/resources/feature-engineering-workflow-resource.yml`)
 - model definition and experiment definition (`ecommerce_user_behaviour/resources/ml-artifacts-resource.yml`)
...
If you want to use different catalog names, please update the `targets` declared in the `ecommerce_user_behaviour/databricks.yml` and `ecommerce_user_behaviour/resources/ml-artifacts-resource.yml` files.
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` deployment target is used by ML engineers to deploy ML resources to development workspace with `dev` configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-ecommerce_user_behaviour-model     | /dev-ecommerce_user_behaviour-experiment     |
| staging     | The `staging` deployment target is part of the CD pipeline. Latest main content will be deployed to staging workspace with `staging` config.                                                             | staging workspace    | staging-ecommerce_user_behaviour-model | /staging-ecommerce_user_behaviour-experiment |
| prod        | The `prod` deployment target is part of the CD pipeline. Latest release content will be deployed to prod workspace with `prod` config.                                                                      | prod workspace       | prod-ecommerce_user_behaviour-model    | /prod-ecommerce_user_behaviour-experiment    |
| test        | The `test` deployment target is part of the CI pipeline. For changes targeting the main branch, upon making a PR, an integration test will be triggered and ML resources deployed to the staging workspace defined under `test` deployment target. | staging workspace    | test-ecommerce_user_behaviour-model    | /test-ecommerce_user_behaviour-experiment    |
...
2. Complete the `TODO` in `ecommerce_user_behaviour/databricks.yml` to add the dev workspace URI under `targets.dev.workspace.host`.
...
```
Following this, update the batch_inference_job base parameters in `ecommerce_user_behaviour/resources/batch-inference-workflow-resource.yml` to pass
...
* [validation.py](../src/validation/validation.py) defines custom metrics and validation thresholds that are referenced by the above resource config files.
* [notebooks/ModelValidation](../src/validation/notebooks/ModelValidation.py) contains the validation job implementation. In most cases you don't need to modify this file.

To set up and enable model validation, update [validation.py](../src/validation/validation.py) to return desired custom metrics and validation thresholds, then 
...
To get started, open `ecommerce_user_behaviour/resources/batch-inference-workflow-resource.yml`.  The file contains the ML resource definition of a batch inference job, like:
...
resources:
  jobs:
    batch_inference_job:
      name: ecommerce_user_behaviour-batch-inference-job
      tasks:
...
The example above defines a Databricks job with name `ecommerce_user_behaviour-batch-inference-job`
that runs the notebook under `ecommerce_user_behaviour/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model for batch inference. 
...
We specify a `batch_inference_job` under `resources/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `ecommerce_user_behaviour-batch-inference-job`.
...
`test-ecommerce_user_behaviour-batch-inference-job`. During the deployment of the `staging` environment config, the job name will be
`staging-ecommerce_user_behaviour-batch-inference-job`.
